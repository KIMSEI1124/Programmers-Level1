...
public class Solution {
    public int[] solution(int N, int[] stages) {
        ...
        int[][] failureArray = new int[N][2];
        for( int i = 0; N > i; i++ ) {
            int countPlayer = 0;    // 스테이지에 도전한 플레이어
            int countFailPlayer = 0;   // 스테이지클리어에 실패한 플레이어
            ...
            if ( countPlayer == 0) {
                ...
                failureArray[i][0] = ((countFailPlayer * 100) / countPlayer);
            }
            failureArray[i][1] = (i+1);
        }
        // failureArray 정렬
        Arrays.sort(failureArray, new Comparator<int[]>() {
            @Override
